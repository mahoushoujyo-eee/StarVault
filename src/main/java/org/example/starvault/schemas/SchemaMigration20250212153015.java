...
public class SchemaMigration20250212153015 extends SchemaMigrationBase {
    @Override
    public void forward() {
        forwardBuilder.alterColumn("user", "avatar",
                ColumnMetadata.builder().name("avatar").type("VARCHAR(1000)").nullable(true).unique(false).build());
    }
    @Override
